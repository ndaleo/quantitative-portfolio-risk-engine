...
# Quantitative Risk Management Engine: Monte Carlo Simulation & Macro Stress-Testing

An institutional-grade quantitative risk modeling framework developed in Python to stress-test a **$1,000,000 multi-asset portfolio**. The engine implements **Cholesky factorization** to preserve cross-asset covariance structures across a **10,000-iteration Monte Carlo simulation**, calculates non-parametric tail-risk metrics (**Value at Risk & Expected Shortfall / CVaR**), and evaluates solvency under **deterministic macroeconomic crisis scenarios**.
...
### 1. Covariance Modeling & Cholesky Decomposition
...
* **Value at Risk ($\text{VaR}_\alpha$):** Quantifies the boundary loss threshold at confidence level $\alpha$ ($95\%$ and $99\%$):
  $$\text{VaR}_\alpha = \text{Percentile}(\text{P\&L}, 1 - \alpha)$$
* **Expected Shortfall ($\text{CVaR}_\alpha$):** Solves the convexity/subadditivity flaw of VaR by computing the conditional expectation of loss exceeding the VaR boundary:
...
## Key Results & Audit Deliverables
